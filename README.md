...
Set the token and the page ID from part 1 as environment variables — nothing containing either one exists as a file anywhere in the repo. Terraform reads any `TF_VAR_<name>` variable automatically, so `TF_VAR_root_page_id` fills `var.root_page_id` with no `.tfvars` file involved:
